"""
Script to run the content ingestion pipeline with proper async support.
"""
import asyncio
import sys
import logging
...
class AsyncContentIngestor:
    """
    Async content ingestion pipeline that processes Docusaurus markdown files
    and populates the database and vector store.
    """

    def __init__(self, db_session: Session):
        """
        Initialize the async content ingestor.
    ...
    async def ingest_content(self, content_info: Dict[str, Any]) -> bool:
        """
    ...
    async def run_ingestion(self, book_path: str) -> int:
        """
        ...
        # Process content with async embeddings
        success_count = 0
        for content_info in contents:
            if await self.ingest_content(content_info):
                success_count += 1
...
async def main():
    """
    Main async function to run the content ingestion pipeline.
    """
    # Set up logging
    ...
    try:
        ...
        ingestor = AsyncContentIngestor(db_session=db)

        # Get book path from command line or use default
        book_path = sys.argv[1] if len(sys.argv) > 1 else "../../../book"

        success_count = await ingestor.run_ingestion(book_path)
